...
function SWEP:DrawHUD()
	...
	local bScope = self.Weapon:GetDTBool(2) or self.Weapon:GetDTBool(1)
	
	self.DrawCrosshair = self.Weapon:GetDTBool(1) and self.DrawCrosshairIronsights
	if self.Weapon:GetDTBool(1) then
        ...
        surface.DrawRect(x - 12, y - 1, 3, 3)
        surface.DrawRect(x + 8, y - 1, 3, 3)
        surface.DrawRect(x - 2, y - 9, 3, 3)
        surface.DrawRect(x - 2, y + 7, 3, 3)
	end
	...
	local Cone = (self.Weapon:GetNWFloat(2) or self.Primary.Cone)
	
	local gap = (Cone * 275 + ((Cone * 275 * (ScrH() / 720)))) * 0.75
...
function SWEP:DrawFuelHUD()
	...
	if (self.Owner:GetNetworkedInt("Fuel") > 0) then
		...
		if not self.BaseClass.WrenchData then
			...
			t.Texture = surface.GetTextureID("vgui/hud/ent_mad_fuel_mini")
...
function SWEP:GetViewModelPosition(pos, ang)
	...
	if (self.Owner:KeyDown(IN_SPEED) or self.Weapon:GetDTBool(0)) and vm:GetSequenceActivityName( vm:GetSequence() ) ~= "ACT_VM_FIDGET" then
...
function SWEP:GetTracerOrigin()
	...
	if (self.Weapon:GetDTBool(1) and not self.DrawCrosshairIronsights) then
